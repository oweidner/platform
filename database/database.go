...
func NewDefaultDatastore(hostname string, dbname string, username string, password string) *DefaultDatastore {
	...
	var connect = fmt.Sprintf("%v:%v@%v/%v?charset=utf8&parseTime=false", username, password, hostname, dbname)
	// connectCleaned ommits the password from the string for security reasons
	var connectCleaned = fmt.Sprintf("%v:%v@%v/%v?charset=utf8&parseTime=false", username, "***", hostname, dbname)
