...
func DBGetAccounts(db *sql.DB, accountID int64) (AccountList, error) {
	...
	if accountID == -1 {
		queryString := `SELECT id, firstname, lastname, email, username FROM platform_account`
		...
		queryString := `SELECT id, firstname, lastname, email, username FROM platform_account WHERE id = ?`
		stmt, err := db.Prepare(queryString)
		if err != nil {
			return nil, err
		}
